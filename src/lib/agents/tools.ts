...
import { z } from "zod"
import { NODECOMMERCE_BASE_URL, NODECOMMERCE_INTERNAL_KEY } from "@/lib/env"

// Shared auth headers for all internal API calls
const AUTH = { Authorization: `Bearer ${NODECOMMERCE_INTERNAL_KEY}` };

export const getDistrictStock = tool(
  async ({ districtId }) => {
    const res = await fetch(
      `${NODECOMMERCE_BASE_URL}/api/districts/${districtId}/stock`,
      { headers: AUTH }
    )
...
export const getSalesHistory = tool(
  async ({ districtId, productCode, days }) => {
    const res = await fetch(
      `${NODECOMMERCE_BASE_URL}/api/analytics/sales?districtId=${districtId}&productCode=${productCode}&days=${days}`,
      { headers: AUTH }
    )
...
export const getMarketPrices = tool(
  async ({ productCode }) => {
    const res = await fetch(
      `${NODECOMMERCE_BASE_URL}/api/market/prices?productCode=${productCode}`,
      { headers: AUTH }
    )
...
export const getWeatherForecast = tool(
  async ({ districtId, days }) => {
    const res = await fetch(
      `${NODECOMMERCE_BASE_URL}/api/weather?districtId=${districtId}&days=${days}`,
      { headers: AUTH }
    )
...
export const getUpcomingEvents = tool(
  async ({ districtId, days }) => {
    const res = await fetch(
      `${NODECOMMERCE_BASE_URL}/api/events/calendar?districtId=${districtId}&days=${days}`,
      { headers: AUTH }
    )
...
export const getHistoricalShipments = tool(
  async ({ fromDistrict, toDistrict, days }) => {
    const res = await fetch(
      `${NODECOMMERCE_BASE_URL}/api/shipments/history?from=${fromDistrict}&to=${toDistrict}&days=${days}`,
      { headers: AUTH }
    )
...
export const postForecastResult = tool(
  async ({ agentName, forecastType, payload }) => {
    const res = await fetch(
      `${NODECOMMERCE_BASE_URL}/api/agent/forecasts`,
      {
        method: "POST",
        headers: {
