...
export function AcoShipmentsPanel() {
  ...
  const fetchShipments = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/seller/aco-shipments");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4">
        <h2 className="text-2xl font-bold text-gray-900">ACO Automated Shipments</h2>
        <p className="text-gray-600">
          These are Phase 1 shipments automatically generated by the Global ACO Pipeline to fulfill regional demand.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
          ...
          <p className="text-gray-500 text-lg">No ACO shipments found for your facility.</p>
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {shipments.map((ship) => (
            <div key={ship.id} className="bg-white border border-blue-200 rounded-xl overflow-hidden shadow-sm flex flex-col relative">
              ...
              <div className="p-5 flex-1 pt-8">
                <div className="flex items-center gap-2 mb-4">
                  <span className="font-bold text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded">
                    Phase {ship.phase}
                  </span>
                ...
                <div className="mb-4">
                  ...
                  <div className="font-bold text-lg text-gray-900">{ship.toName}</div>
                  <div className="text-sm text-gray-600 capitalize">{ship.toType.replace("_", " ")}</div>
                </div>
